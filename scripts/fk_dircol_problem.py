...
class FKDircolProblem(DircolProblem):
  def __init__(self, Q, R, steps, dt, dynamics, dynamics_deriv, nx, nu, xinit, ulb, uub, pterm, pfk, pjacobian, nq, nv):
    super().__init__(Q, R, steps, dt, dynamics, dynamics_deriv, nx, nu, xinit, None, ulb, uub)
    self.pterm = pterm
    self.pfk = pfk
    self.pjacobian = pjacobian
    self.nq = nq
    self.nv = nv

  def get_num_constraints(self):
    return self.nx*self.steps + 2

  def solve(self):
    num_decision_vars = self.get_num_decision_vars()
    warm_start = np.zeros(num_decision_vars)

    warm_start[:self.nx] = self.xinit
  ...
  def constraints(self, x):
    ...
    # Terminal ee position
    q = x[-self.vars_per_step:-self.vars_per_step + self.nq]
    constraint_vals[-2:] = self.pfk(q) - self.pterm
  ...
  def jacobian(self, x):
    ...
    q = x[-self.vars_per_step:-self.vars_per_step + self.nq]
    vals = np.append(vals, self.pjacobian(q).flatten(), 0)
  ...
  def jacobianstructure(self):
    ...
    # Terminal constraint
    rows.extend([self.get_num_constraints() - 2]*self.nv + [self.get_num_constraints() - 1]*self.nv)
    last_state_col = self.get_num_decision_vars() - self.vars_per_step
    cols.extend([last_state_col + i for i in range(self.nv)]*2)
